...
let s:concealends = ''
let s:concealcode = ''
let s:concealcodeblock = ''
if has('conceal') && get(g:, 'vim_markdown_conceal', 1)
  let s:conceal = ' conceal'
  let s:concealends = ' concealends'
endif
if has('conceal') && get(g:, 'vim_markdown_conceal_code_inline', 1)
  let s:concealcode = ' concealends'
endif

if has('conceal') && get(g:, 'vim_markdown_conceal_code_blocks', 0)
  let s:concealcodeblock = ' concealends'
endif
...
execute 'syn region mkdCode matchgroup=mkdCodeDelimiter start=/^\s*\z(`\{3,}\)[^`]*$/                       end=/^\s*\z1`*\s*$/'            . s:concealcodeblock
execute 'syn region mkdCode matchgroup=mkdCodeDelimiter start=/\(\([^\\]\|^\)\\\)\@<!\~\~/  end=/\(\([^\\]\|^\)\\\)\@<!\~\~/'               . s:concealcode
execute 'syn region mkdCode matchgroup=mkdCodeDelimiter start=/^\s*\z(\~\{3,}\)\s*[0-9A-Za-z_+-]*\s*$/      end=/^\s*\z1\~*\s*$/'           . s:concealcodeblock
...
execute 'syn region mkdStrike matchgroup=CmpItemAbbrDeprecated start="\%(\~\~\)" end="\%(\~\~\)"' . s:concealends
execute 'syn region mkdBoldItalic matchgroup=markdownBoldItalic start="\%(\*\*_\)" end="\%(_\*\*\)"' . s:concealends
...
HtmlHiLink mkdFootnotes     markdownLinkText 
HtmlHiLink mkdLink          markdownLinkText 
HtmlHiLink mkdURL           markdownUrl 
...
HtmlHiLink mkdStrike        CmpItemAbbrDeprecated 
HtmlHiLink mkdBoldItalic    markdownBoldItalic 
